...
- The report function is `wiseurl_analytics_report(p_from date, p_to date, p_source text default null, p_link_id uuid default null, p_group_id uuid default null, p_as_of timestamptz default null) returns jsonb`.
- The raw-row function is `wiseurl_analytics_clicks(p_from date, p_to date, p_source text default null, p_link_id uuid default null, p_group_id uuid default null, p_as_of timestamptz default null, p_offset integer default 0, p_limit integer default 50) returns table (...)`. The first report returns a range-end snapshot; pagination and CSV reuse it for both RPCs so new traffic cannot shift rows or totals between pages.
