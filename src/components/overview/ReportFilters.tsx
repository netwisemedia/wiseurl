...
export default function ReportFilters({ filters, groups, update }: {
  ...
  return <section className="report-filters" aria-label="Report filters">
    ...
    <div className="flex flex-wrap gap-3 items-end mt-4 pt-4 border-t border-[var(--border)]">
      ...
      <label className="field-label flex-1 min-w-40">Traffic<select className="input" value={filters.traffic} onChange={e => update({ traffic: e.target.value })}>
        <option value="non_bot">Exclude detected bots</option><option value="all">All recorded clicks</option><option value="bots">Detected bots only</option>
      </select></label>
      <label className="field-label flex-1 min-w-40">Link group<select className="input" value={filters.groupId || ''} onChange={e => update({ group: e.target.value || null })}>
